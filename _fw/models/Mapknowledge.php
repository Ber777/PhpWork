...
class Mapknowledge
{
	...
	var $DB = 0; //флаг Базы данных (если $this->DB = 1, то значит не KZ, а БД)
	...
	public function get_user_templates($id) {
		$link = dbConnect();
		$query ="SELECT o_id, o_name, o_id_type, user_id, user_id_system FROM nir.all_templates_view where o_id_type=$id and user_id_system=current_user order by o_name";
	...
	//Смена имени КЗ
	public function changeNameKZ()
	{
		$this->id = (int) $this->id;
		$this->name = stripslashes($this->name);
		$query = pg_query($this->link, "SELECT * FROM nir.changenamekz($this->id, '$this->name')");
		$result = pg_fetch_array($query);
		//запись лога
		$action = 'Изменение имени КЗ';
		$inputs = 'ID КЗ: '.$this->id.', новое имя КЗ: '.$this->name;
		if($result[0] == 1)
			$res = 'Успех';
		else
			$res = 'Ошибка';
   ...
   public function getList()
    {
		...
		while($data = pg_fetch_array($query))
		{
			$list[] = array("id" => $data['idkz'], "name" => $data['namekz']);
		}
		return $list;
    } 
    public function geReadList()
    {
		$this->user_id = (int) $this->user_id;
        $query = pg_query($this->link, "SELECT * FROM nir.get_kz($this->user_id) where (select isReader from nir.get_access(idkz))");
		$list = array();
		while($data = pg_fetch_array($query))
		{
			$list[] = array("id" => $data['idkz'], "name" => $data['namekz']);
	...
	{
		$this->user_id = (int) $this->user_id;
		$query = pg_query($this->link, "SELECT * FROM nir.all_kzs_view where ((select isReader from nir.get_access(o_id)) or (select isOwner from nir.get_access(o_id))) ORDER BY o_name");
		$kz = array();
		while($data = pg_fetch_array($query))
		{
			$kz[] = array("id" => $data['o_id'], "name" => $data['o_name'], "isOwner" => $data['isowner'], "user_id" => $data['user_id'], "iuser_name" => $data['user_name']);
