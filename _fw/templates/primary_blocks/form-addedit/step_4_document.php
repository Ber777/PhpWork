...
<form action="" method="POST" id="main-form" enctype="multipart/form-data" class="block-step _text-align-left _display-block" ajax="<?=$this->url_ajax_handler ?>" id_object="<?=$this->id_for_ajax ?>">
    ...
    <ul class="list-float-none" id="list-fields">
        <li><label for="">Имя файла: </label><input itype="2" name="namef" class="standart-input" type="text" placeholder="введите имя файла" value="<?=$this->name_document ?>" required></li>
    ...
    <div class="_center-in-div" style="margin-bottom: 10px;">
        <? if ($this->path_file): ?>
            <p>Ссылка на файл: <a target="_blank" class="_underline name-file" href="<?=$this->path_file ?>"><?=$this->name_file ?></a></p>
        <? endif; ?>
            <p class="_padding-for-h1"><input delete="<?=$this->path_file ?>" type="file" class="click-button" its="docs" name="doc" id="doc"></p>
